...
export function SiteFooter() {
  return (
    <footer className="mt-[150px] border-t border-[rgba(12,10,8,0.12)] px-[40px] pt-[38px] pb-[64px] max-[800px]:mt-[80px] max-[800px]:px-[20px] max-[800px]:pt-[30px] max-[800px]:pb-[48px]">
        ...
        <ul className="flex flex-wrap items-center justify-center gap-x-[32px] gap-y-[10px] max-[800px]:gap-x-[22px]">
          {LINKS.map((link) => (
            <li key={link.label}>
              <Link
                href={link.href}
                className="font-[family-name:var(--font-inter)] text-[15px] leading-[22px] text-[rgba(12,10,8,0.75)] transition-colors duration-200 hover:text-[#0c0a08] max-[800px]:text-[14px]"
              >
                {link.label}
              </Link>
            </li>
      ...
      <p className="mx-auto mt-[22px] max-w-[760px] text-center font-[family-name:var(--font-inter)] text-[13px] leading-[21px] text-[rgba(12,10,8,0.45)] max-[800px]:text-[12px] max-[800px]:leading-[19px]">
        {DISCLAIMER}
      </p>
    </footer>
  );
