...
function PersonCard({ name, role, initial, bio, photo }: Person) {
  return (
    <article className="overflow-hidden rounded-[12px] border border-[#F5F5F5] bg-[#0B0B0B]">
      <div className="bg-[#F5F5F5] px-[16px] py-[12px] text-center font-[family-name:var(--font-pt-serif)] text-[14px] leading-[18px] font-bold text-[#0B0B0B]">
        {name}, {role}
      </div>
      <div className="flex flex-col items-center px-[18px] py-[28px]">
        <div className="relative h-[112px] w-[112px] overflow-hidden rounded-full border border-[#F5F5F5] bg-[#0B0B0B]">
          {photo ? (
            <>
            ...
            <span className="flex h-full w-full items-center justify-center font-[family-name:var(--font-pt-serif)] text-[28px] font-bold text-[#F5F5F5]">
              {initial}
            </span>
          )}
        </div>
        <p className="mt-[20px] text-center font-[family-name:var(--font-inter)] text-[13px] leading-[18px] text-[#F5F5F5]">
          {bio}
        </p>
      </div>
    </article>
    ...
    <>
      <div className="hidden min-[801px]:block">
        <section className="mt-[220px] px-[40px] py-[5px]">
          <div className="mx-auto w-full max-w-[1120px]">
            <h2 className="text-center font-[family-name:var(--font-pt-serif)] text-[48px] leading-[52px] font-bold text-[#F5F5F5]">
        ...
        <section className="mt-[50px] px-[40px] py-[50px]">
          <div className="mx-auto w-full max-w-[1120px]">
            ...
            <div className="mt-[40px] rounded-[12px] bg-[#F5F5F5] px-[40px] py-[48px]">
              <div className="grid grid-cols-3 gap-[24px] text-center">
                {STATS.map((stat) => (
                  <div key={stat.n}>
                    <div className="font-[family-name:var(--font-pt-serif)] text-[56px] leading-[64px] font-bold text-[#0B0B0B]">
                      {stat.n}
                    </div>
                    <p className="mt-[12px] font-[family-name:var(--font-inter)] text-[14px] leading-[20px] text-[rgba(11,11,11,0.7)]">
                      {stat.label}
                    </p>
                  </div>
                ))}
      ...
      <div className="max-[800px]:block min-[801px]:hidden">
        ...
        <section className="mt-[40px] flex flex-col gap-[20px] px-[20px]">
          ...
          <div className="rounded-[12px] bg-[#F5F5F5] px-[20px] py-[32px]">
            <div className="flex flex-col gap-[28px] text-center">
              {STATS.map((stat) => (
                <div key={stat.n}>
                  <div className="font-[family-name:var(--font-pt-serif)] text-[40px] leading-[48px] font-bold text-[#0B0B0B]">
                    {stat.n}
                  </div>
                  <p className="mt-[8px] font-[family-name:var(--font-inter)] text-[14px] leading-[20px] text-[rgba(11,11,11,0.7)]">
                    {stat.label}
                  </p>
                </div>
              ))}
